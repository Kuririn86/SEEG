...
状态识别、起始点定位和通道排序在统一机制链上联合学习。帧级状态头判断某一时刻是否处于发作状态，边界头定位状态转变，通道募集头估计各通道在 onset 附近被异常募集的时间和强度；片段标签与概率由状态序列聚合得到，onset 由状态和边界证据联合确定，Top-10 则由早期募集、持续演化和网络传播特征产生。该设计避免将通道解释降为分类完成后的附加可视化。
...
+\lambda_5\mathcal{L}_{\mathrm{channel}}
+\lambda_6\mathcal{L}_{\mathrm{reg}}.
$$

其中，片段损失采用类别加权 BCE 或非对称 Focal Loss；状态损失按可用标注采用 Focal BCE 与 Soft Dice；onset 损失使用以专家标注为中心的窄高斯软标签，降低逐帧硬边界对标注差异的敏感性；状态转移损失惩罚孤立高概率和不合理的频繁切换，并约束边界后具有最小持续时间；通道损失对募集弱标签或人工通道标注进行学习；正则项包括动态图稀疏、方向一致性和权重衰减。
...
实验设置 Baseline、机制增强模型和轻量模型三类。Baseline 采用固定预处理、单分支一维 CNN、全局池化和 BCE；机制增强模型采用状态—边界—募集联合头与动态双图；轻量模型为经蒸馏和目标硬件优化后的部署版本。
...
| 通道关系 | 无图、静态相关图、动态功能图、解剖—功能双图 | F1、事件敏感性、传播排序、内存 |
| 长程模块 | 全局池化、TCN、TCN 加轻量注意力 | AUC、onset MAE |
| 联合输出头 | 仅片段级、状态头、状态+边界、状态+边界+募集 | AUC、onset MAE、Top-10 稳定性 |
| 类别不平衡 | BCE、加权 BCE、Focal、困难负样本 | Recall、Specificity |
| onset 后处理 | 单阈值、平滑、滞回、变化点校正 | MAE、假 onset 率 |
| 通道排序 | 分类贡献、加入早期性、加入图外流、完整快速评分 | Hit@10、NDCG、跨发作一致性 |
| 解释链路 | 快速一次前向、积分梯度、遮挡验证 | 排名一致性、端到端时延 |
...
目标用户为癫痫监测单元神经电生理医生、癫痫专科医生及术前多学科团队。系统用于长时程 SEEG 记录完成后的批量筛查，或监测过程中的准实时事件提示：从连续记录中标记疑似发作片段，给出候选 onset，展示当前植入覆盖范围内的起始候选和传播通道，并在同一患者存在多次发作时汇总跨发作一致性。其作用是缩小需要人工精读的数据范围，提供可追溯的电生理证据，不替代原始波形复核。

系统不独立确诊癫痫，不自动定义致痫区，不决定手术切除或消融边界，不替代 MRI、PET、症状学、功能定位及多学科决策。Top-10 仅表示当前记录和当前植入范围内与状态转变相关的重要通道；未被电极覆盖的脑区无法由模型排除或定位。临床展示应同时给出发作概率、onset 及其置信区间、起始与传播分层、主要时频/网络证据和数据质量提示。
...
本方案的主要优势在于将比赛四类输出统一到“状态转变—早期募集—动态传播”机制链。多尺度时域与时频编码覆盖瞬态放电和持续节律演化；动态双图刻画基线、起始和传播阶段的网络变化；状态与边界联合输出提高 onset 的时间可辨性；募集头和早期通道评分减少晚期高振幅传播通道对 Top-10 的干扰。正式通道排序基本由一次前向完成，昂贵归因仅用于离线验证，使解释性与低时延要求相容。
...
本方案面向多通道 SEEG 发作状态检测，提出质量感知的状态转变与动态传播联合网络 QST-DPGNet。模型以多尺度时域—时频编码为基础，通过解剖—功能动态双图描述发作前基线、onset 早期和传播阶段的网络变化，并设置片段状态头、帧级状态头、onset 边界头和通道募集头，在统一机制链上生成二分类标签、六位小数发作概率、片段内 onset 及 Top-10 通道。onset 由状态持续性、边界峰值和变化点证据联合确定；阳性通道排序限定在 onset 早期窗口，综合募集强度、激活潜伏期、节律演化、动态图外流、跨发作重复性和伪迹风险，减少晚期传播通道对排序的干扰。正式推理采用一次前向的快速评分，积分梯度、遮挡和扰动分析仅用于离线可信度验证。方案通过患者级隔离、分级标签监督、概率校准、稀疏动态图、知识蒸馏和滑窗缓存控制泛化误差与计算成本，并明确将输出限定为当前植入覆盖范围内的阅片辅助证据，不将 Top-10 直接解释为致痫区或手术靶点。
